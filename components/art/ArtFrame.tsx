...
import { ART, type ArtKey, isArtPending } from "./registry";
...
export function ArtFrame({ artKey, children, className = "", eager = false, ratio = "aspect-video" }: { artKey: ArtKey; children?: ReactNode; className?: string; eager?: boolean; ratio?: string }) {
  ...
  // Pending keys have no shipped WebP yet — render the plate only, never the <Image>,
  // so there is no 404/400 through next/image (D-052 plate-as-primary fallback).
  const showImage = !failed && !isArtPending(artKey);
  return (
    <div className={`art-frame relative isolate overflow-hidden rounded-card border border-line ${ratio} ${className}`}>
      ...
      {showImage && <Image src={art.src} alt={art.alt} fill sizes="(max-width: 768px) 100vw, 720px" priority={eager} onError={() => setFailed(true)} className="object-cover" />}
