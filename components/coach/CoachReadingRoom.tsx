  ...
  return (
    <div className="mx-auto max-w-[38.75rem] px-4 pb-40">
      <section className="pt-5" aria-busy={briefState === "loading"}>
        ...
        {topAdaptations.length > 0 && <div className="mt-4 flex flex-wrap gap-2">{topAdaptations.map((item) => <Chip key={item.id} onClick={() => setOpen(item.id)} className="max-w-full whitespace-normal break-words text-left">{item.label}{item.status === "proposed" ? " · pending" : ""}</Chip>)}</div>}
      </section>
            ...
            {view.thread.map((message) => {
              ...
              return (
                <div key={message.id} className="max-w-[85%]">
                  ...
                  {proposed && <div className="mt-2"><Chip onClick={() => setOpen(proposed.id)} className="max-w-full whitespace-normal break-words text-left">{proposed.label}{proposed.status === "proposed" ? " · pending" : ""}</Chip></div>}
                </div>
              );
            })}
          </div>
