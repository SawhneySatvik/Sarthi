...
| **For** | The next action: plan `SAR-007` (F3 fake-stack integration eval — the GATE), then the demo-spine fan-out (SAR-008/009/010) + the Phase-1 sellable wrap |
| **Read order** | AGENTS.md → docs/product/PROJECT.md → docs/architecture/TECH-STACK.md → docs/experience/FLOWS.md → rest as needed |
| **State of play** | Phase-0/1 contract reconciled. 40 decisions, signed architecture, and locked tickets; `SAR-001`–`SAR-004` (through the capture moat) are accepted, and **`SAR-005` (token shell + Today spine) + `SAR-006` (Health slice + capture-sheet hero — the F3 loop) are landed (code)**. The full F3 loop runs keyless end-to-end: parse → route-by-confidence → filed-automatically strip + estimate deck (4 gestures) → typed Health write through the real SAR-004 commit service → XP + inline level-up → 5-min undo → Today `via capture`; plus the Health lens (3 rings). Validated: `pnpm check` **101 tests**, build, invariants, boundary; route-loop + render smokes; a **two-pass Sol review** (4 blocking findings — phantom strip rows, undo divergence, amber-on-orb, auto-batch provenance — fixed) ACCEPTABLE TO LAND. The server re-routes the auto batch (D-040) so no estimated value writes without an explicit accept. **SAR-006's one open DoD is visual screenshot-verify before it CLOSES; SAR-007 is the formal F3 eval gate.** All commits stack on branch `sar-003-schema-repository`. |
