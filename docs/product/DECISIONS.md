...
## 2026-07-18 — Meal/receipt vision path (SAR-011)

**D-045 · Photo capture flows through the same estimate-card loop; photo evidence NEVER auto-writes.**
SAR-011 wires the camera path keyless on the fake vision stack: the capture bar's camera button → a photo `preview` phase (large preview + a meal/receipt TYPE TOGGLE) → `POST /api/capture/parse-photo` (FormData) → `getSession().vision` (`FakeVisionProvider`, no real key) → the pure `core/capture/vision.ts` mapper (`draftFromVisionResult`: meal → one forced-`estimated` proposal; receipt → N **integer-paise** transaction proposals) → the UNTOUCHED `routeDraft` → estimate deck → typed writes (meal → Health, receipt → Money batch, FLOWS F5). **The trust rule (one routing addition):** any proposal carrying photo evidence (`evidenceRefs.length > 0`) routes to **pending** — so a photographed meal or receipt, even at explicit high confidence, is NEVER auto-written; the commit route's server-side re-route (D-040) re-enforces this against a mislabelling client (proven by the `f5-receipt` eval, `wrongSilentWrites === 0`). The text F3 flow is unchanged (the shared post-parse routine `ingestDraft` is byte-for-byte the prior inline path — Sol-verified). **Signed UX choice (Satvik):** meal-vs-receipt is a TYPE TOGGLE in a preview-then-parse photo phase — this **supersedes SCREEN-CAPTURE §2.4's "chips-are-hints, not modes"** for the photo type (chosen for a demo-safe live snap on the toggle-driven fake stack); on the fake stack the toggle is the SOLE meal/receipt driver (the filename cue was removed + tested). The shared capture sheet moved to `--bg-raised` + `--elev-card` (proper elevation over the scrim — improves both text and photo paths). **Deferred follow-ups (recorded):** F5 merchant/date context on the receipt confirm (a non-write context row, or reconcile FLOWS F5 with the no-auto-write rule); a §4b full-bleed photo region on the meal estimate card; a confirm-sheet min-height (so it grows, not shrinks, on Analyze); a direct route-handler test of the photo-auto refusal + the pre-buffer size cap; real content-based vision (a later ticket — the toggle is the keyless stand-in). · **Locked**

**D-046 · Onboarding is the real new-account path — a keyless answer-derived spine generator + one atomic accept write; the whole SCREEN-ONBOARDING flow ships.**
SAR-012 builds the full onboarding (A welcome → B core B1–B6 → C spine generation → D confirm & trim → E DETAIL → F theme → G landing), gated in three passes. **Routing (signed):** `app/(app)/layout.tsx` is async — a missing profile or `onboardingStatus !== 'complete'` redirects the whole `(app)` subtree to `/onboarding`; the D-accept write flips the status (the commit signal); `seed-dev` writes a `complete` profile for `populated|empty|alldone` (Today demo untouched) and a new `SEED_STATE=fresh` is the onboarding-landing state. **Spine gen (C):** a real deep-tier `generateObject` per selected domain through the injected `LlmGateway` (op `onboarding-spine`) with a deterministic **answer-derived** `deriveSpine` (`core/onboarding/spine.ts`, under-18 branch); the fake adapter parses a locked prompt envelope so keyless spines genuinely vary by answer; per-domain parallel + 8s budget + independent retry; StrictMode-safe. **The ONE atomic accept write (D):** profile → money categories/budgets → habits/rules → skills/milestones → per-domain plan arcs + Day-1 items → zeroed progress/snapshots → `profile_gaps` (5 DETAIL + un-selected domains) → a deterministic Day-1 `coach.notes` line (signed **D-F(h)**) → flip `onboardingStatus:complete`. Replay-safe (in-txn status re-check → clean replay; intra-payload duplicate names collapse; schema rejects empty/duplicate-domain spines + bad timezones at the boundary), integer units, **nothing writes before the confirm tap**. **E DETAIL** answers/resolves the accept-created `profile_gaps` (never creates them; skip leaves them open for the SAR-014 daily brief); E5 income/bills seed integer-paise recurring rules; the detail apply is single-flight. **F theme** live-applies + persists via the existing `sarthi-theme` mechanism (real per-theme previews; app-mode-aware selected ring). **G landing** crosses to Today Day-1 with the dismissible mic hint + the coach first line, plan populated from the spine. **Demo gesture (signed D-G):** long-press the welcome wordmark → `seed_runs`-idempotent `seedDemo` (shared with `seed-dev`), reachable only in local mode (404 otherwise). **Deferrals (recorded):** the daily-brief gap CONSUMPTION is SAR-014; auth sign-in + the authenticated demo-clone are SAR-021+; real voice audio is SAR-013; **voice-fill parity on the E DETAIL sections is deferred** (chip/slider-only in v1 — optional, skippable, gap-backfilled; E5 voice-first is the highest-value follow-up). The `--energy` CORE progress hairline on the A→D plan-creation arc is the **sanctioned SCREEN-ONBOARDING §1 exception** to the amber-only-when-earned rule (hidden on the post-accept polish phases). Carry-forward: the seed-route guard rests on `AUTH_PROVIDER` — **SAR-021's deploy checklist MUST assert `supabase`**. **Pass 3 was closed at Satvik's call without the final screenshot re-review loop** (two residual polish items — Today section-eyebrow AA + the E-grid Done CTA height — fixed directly; `pnpm check` 192 green). · **Locked**

---

## 2026-07-19 — SAR-019A visual narrative refinement

**D-047 · Journey memory → bounded daily reflections and local-first image media, never a fifth domain.**
`daily_reflections` records are explicit, user-scoped memory metadata: one row per local day, fixed mood, integer energy, optional integer sleep minutes, journal text, and grounded summary provenance. `reflection_media` contains at most four image attachments per reflection. A provider-blind MediaProvider stores private local files for the keyless fake/dev track and leaves a Supabase-shaped adapter seam for SAR-021; clients never submit `userId`. These records create no XP, plan effect, typed-domain row, automatic coach adaptation, or generic event log. Images only in web v0; video waits for the native-mobile phase. · **Locked**

**D-048 · Today → every remaining task is an art-led, individually swipeable action card.**
This supersedes D-014's one-action-only presentation rule, not its plan-forward purpose: each pending task remains a real scoped plan item, right swipe performs existing Done and left swipe existing Skip, and each card keeps visible 44px/keyboard-equivalent controls. Status changes continue through the existing manual action and keep their capture-context, XP, completed-settle, and no-estimate-write behavior. · **Locked**
